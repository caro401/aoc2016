...
def decrypt(s, id):
    n = [ alpha[(alpha.index(a)+id)%26] if a in alpha else " " for a in s]
    return "".join(n)

total = 0
for line in input.split("\n"):
    bits = re.search("([-a-z]+)([0-9]+)\[([a-z]{5})\]", line).groups()
    bits2 = fivemost(bits[0])

    if bits2 == bits[2]:
        total += int(bits[1])
        d = decrypt(bits[0], int(bits[1]))
        for thing in ["bunny", "rabbit", "scavenger"]:
            if thing in d:
                break
        else:
            print d, bits[1]
