...
screen = numpy.zeros((6,50),dtype='int8')

for line in input:
    dg = [int(s) for s in re.findall('[0-9]+',line)]
    if "rect" in line:
        screen[:dg[1],:dg[0]] = 1
    elif "rotate r" in line:
        screen[dg[0],:] = numpy.roll(screen[dg[0],:], dg[1])#, axis = 1)
    elif "rotate c" in line:
        screen[:, dg[0]] = numpy.roll(screen[:,dg[0]], dg[1])#, axis = 0)
